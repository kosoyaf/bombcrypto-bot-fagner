...
def main():
    ...
    while True:
        ...
        if now - last["check_for_captcha"] > addRandomness(t['check_for_captcha'] * 60, randint(1,10)):
            last["check_for_captcha"] = now
            solveCaptcha()

        if now - last["heroes"] > addRandomness(t['send_heroes_for_work'] * 60, randint(1,10)):
            last["heroes"] = now
            refreshHeroes()

        if now - last["login"] > addRandomness(t['check_for_login'] * 60, randint(1,10)):
            sys.stdout.flush()
            last["login"] = now
            login()
        ...
        if now - last["refresh_heroes"] > addRandomness( t['refresh_heroes_positions'] * 60, randint(1,10)):
            solveCaptcha()
